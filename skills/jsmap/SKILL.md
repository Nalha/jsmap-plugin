---
name: jsmap
description: Explore JavaScript/TypeScript codebases compactly. Lists a file's API, maps name-based call graphs, finds candidate callers of uniquely named functions, locates which function contains a string, and extracts one function with callee signatures. Use before reading large .js/.ts files when answering "what's in this file", "where is X", "what does X call", or "which function contains this line". Handles .js, .ts, .mts, .cts, and .tsx.
---
...
Paths are files or directories (directories recurse `.js`/`.ts`/`.tsx`, skipping `node_modules`).
Use **relative, forward-slash** paths — `web`, `web/model/units.js`. Never absolute
Windows paths (`C:\...`); the shell eats the backslashes.

Prefer these compact views before reading a large source file. Read the reported span when
